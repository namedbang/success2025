...
// Public functions
//-------------------------------------------------------------------------------------------------
void initialize_serial_port_info(Serial_Port_infom *info)
{
...
int SerialPortOpen(char *String_Device_File_Name, unsigned int Baud_Rate, TSerialPortParity Parity, TSerialPortID *Pointer_Serial_Port_ID)
{
	...
	if (Serial_Port_File_Descriptor == -1)
		goto Exit_Error;
	...
	// Set speeds
	if (cfsetispeed(&Serial_Port_Parameters, Speed) == -1)
		goto Exit_Error;
	if (cfsetospeed(&Serial_Port_Parameters, Speed) == -1)
		goto Exit_Error;

	// Set parameters
	if (tcsetattr(Serial_Port_File_Descriptor, TCSANOW, &Serial_Port_Parameters) == -1)
		goto Exit_Error;
...
Exit_Error:
	if (Serial_Port_File_Descriptor != -1)
		close(Serial_Port_File_Descriptor); // Close the serial port if it was previously opened
	return -1;
}
...
{
	if (read(Serial_Port_ID, Pointer_Available_Byte, 1) == 1)
		return 1;
	return 0;
}
